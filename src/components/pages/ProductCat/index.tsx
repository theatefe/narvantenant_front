...
// API **************************************************
import GetAllProductCatApi from '../../api/ProductCat/GetAll';
// MUI **************************************************
...
// MODELS ******************************************************
import CreateProductCatModal from '../../ui/modals/productCat/CreateProductCatModal';
import DeleteProductCatModal from '../../ui/modals/productCat/DeleteProductCatModal';
// OTHER *******************************************************
import {
  jalaliDate,
  jalaliDateWithTime,
...
const ProductCatList = () => {
  ...
  const getProductCatList = async () => {
    const list = await GetAllProductCatApi(token);
    if (list.status === 403) {
      setTimeout(() => {
    ...
    if (list.status === 200) {
      const arr = list.data.map((item, index: number) => ({
        ...
        option: (
          <>
            {permissions.find((p) => p.operationId === 'tenantListProduct') ?
              <Tooltip title="لیست محصولات" arrow>
            ...
            {permissions.find((p) => p.operationId === 'tenantUpdateProductCategory') ?
            ...
            {permissions.find((p) => p.operationId === 'tenantDeleteProductCategory') ?
  ...
  React.useEffect(() => {
    dispatch(
      setMetaData({
        title: 'نارون - مدیریت دسته بندی محصولات ',
          ...
          {isLoaded ? <div className="w-full  bg-white shadow-lg rounded-lg border border-gray-200 dark:border-gray-700 dark:bg-gray-800">
            <div className="p-4">
              ...
              <div className="row mb-4">
                <div className="col-6 text-right"><h3 className="text-2xl font-bold text-gray-700 dark:text-gray-200 float-left">مدیریت دسته بندی محصولات </h3></div>
                <div className="col-6 text-left">
                  {
                    permissions.find((p) => p.operationId === 'tenantCreateProductCategory') ?
                      <Button
                        onClick={() => {
                          setModal(true);
                        }}
                ...
                <CreateProductCatModal
                  list={getProductCatList}
                  token={token}
                  id={selectedCatId}
                  openModal={modal}
                  setOpenModal={closeModal}
                />
                <DeleteProductCatModal
                  list={getProductCatList}
                  token={token}
                  id={selectedCatId}
