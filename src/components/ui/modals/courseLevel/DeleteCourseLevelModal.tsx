...
// API *************************************************
import DeleteCourseLevelApi from '../../../api/CourseLevel/Delete';
...
const DeleteCourseLevelCatModal = (props) => {
  ...
  const handleDeleteCourseLevel = async () => {
    if (id) {
      ...
      if (deleted.status === 200) {
        toast.SuccessNotify('سطح آموزشی مورد نظر با موفقیت حذف شد');
        handleCancel();
        setSending(false);
        list();
      } else {
  ...
  const getCourseLevel = async () => {
    if (id) {
      try {
        const courseLevelCat = await GetCourseLevelApi(token, id);
        if (courseLevelCat.status === 200) {
          setCourseLevel(courseLevelCat.data);
        } else {
      ...
      <Box sx={style} justifyContent="center" alignItems="center">
        ...
        <Grid container spacing={2} columns={{ xs: 12, sm: 12, md: 12 }}>
          <Grid item xs={12} md={12} sx={{ mx: 'auto' }}>
            <Typography>
              آیا برای حذف مقطع آموزشی با عنوان <span className='text-danger'>{courseLevel?.title}</span> اطمینان دارید؟
            </Typography>
          </Grid>
        </Grid>
        <Grid
