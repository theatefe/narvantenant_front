...
// API *************************************************
import DeleteSkillApi from '../../../api/Skill/Delete';
...
const DeleteCourseLevelCatModal = (props) => {
  ...
  const handleDeleteCourseLevelCat = async () => {
    if (id) {
      ...
      const deleted = await DeleteSkillApi(token, body);
      if (deleted.status === 200) {
        toast.SuccessNotify('مهارت مورد نظر با موفقیت حذف شد');
        handleCancel();
        setSending(false);
        list();
      } else {
      ...
      <Box sx={style} justifyContent="center" alignItems="center">
        ...
        <Grid container spacing={2} columns={{ xs: 12, sm: 12, md: 12 }}>
          <Grid item xs={12} md={12} sx={{ mx: 'auto' }}>
            <Typography>
              آیا برای حذف مهارت با عنوان <span className='text-danger'>{skill?.title}</span> اطمینان دارید؟
            </Typography>
          </Grid>
        </Grid>
        <Grid
