...
// API *************************************************
import DeleteCourseLevelApi from '../../../api/CourseLevelCat/Delete';
...
const DeleteCourseLevelCatModal = (props) => {
  ...
  // HOOKS FORM **************************************************
  const [courseLevelCat, setCourseLevelCat] = React.useState(null);
  ...
  const handleDeleteCourseLevelCat = async () => {
    if (id) {
      ...
      const deleted = await DeleteCourseLevelApi(token, body);
      if (deleted.status === 200) {
        toast.SuccessNotify('دسته بندی سطح آموزشی با موفقیت حذف شد');
        handleCancel();
        setSending(false);
        list();
      } else {
    ...
    if (id) {
      try {
        ...
        if (courseLevelCat.status === 200) {
          setCourseLevelCat(courseLevelCat.data);
        } else {
      ...
      <Box sx={style} justifyContent="center" alignItems="center">
        ...
        <Grid container spacing={2} columns={{ xs: 12, sm: 12, md: 12 }}>
          <Grid item xs={12} md={12} sx={{ mx: 'auto' }}>
            <Typography>
              آیا برای حذف دسته بندی با عنوان <span className='text-danger'>{courseLevelCat?.title}</span> اطمینان دارید؟
            </Typography>
          </Grid>
        </Grid>
        <Grid
