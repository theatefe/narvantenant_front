...
const DeleteProductModal = (props) => {
  ...
  const handleDeleteProduct = async() => {
    if (id) {
      ...
      const deleted = await DeleteProductApi(token, body);
      if (deleted.status === 200) {
        toast.SuccessNotify('محصول با موفقیت حذف شد');
        handleCancel();
        setSending(false);
        list();
      } else {
      ...
      <Box sx={style} justifyContent="center" alignItems="center">
        ...
        <Grid container spacing={2} columns={{ xs: 12, sm: 12, md: 12 }}>
          <Grid item xs={12} md={12} sx={{ mx: 'auto' }}>
            <Typography>
              آیا برای حذف محصول  <span className='text-danger'>{product?.title}</span> اطمینان دارید؟
            </Typography>
          </Grid>
        </Grid>
        <Grid
