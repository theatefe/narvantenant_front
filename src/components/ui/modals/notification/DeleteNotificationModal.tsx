...
// API *************************************************
import DeleteNotificationApi from '../../../api/Notification/Delete';
 ...
 token, id, openModal, setOpenModal, list
  ...
  const handleDeleteNotification = async() => {
    if (id) {
      ...
      const deleted = await DeleteNotificationApi(token, body);
      if (deleted.status === 200) {
        toast.SuccessNotify('اعلان موردنظر با موفقیت از سیستم حذف شد');
        handleCancel();
        setSending(false);
        list();
      } else {
      ...
      <Box sx={style} justifyContent="center" alignItems="center">
        ...
        <Grid container spacing={2} columns={{ xs: 12, sm: 12, md: 12 }}>
          <Grid item xs={12} md={12} sx={{ mx: 'auto' }}>
            <Typography>
              آیا برای حذف اعلان <span className='text-danger'>{notification?.title}</span> اطمینان دارید؟
            </Typography>
          </Grid>
        </Grid>
        <Grid
